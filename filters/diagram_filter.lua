...
local function py2image(code, filetype)
  ...
  local command = pythonActivatePath and pythonActivatePath .. " && " .. pycmd or pycmd
  os.execute(command)
...
function CodeBlock(block)
  ...
  local success, img = pcall(img_converter, block.text, filetype, block.attributes["additionalPackages"] or nil)
